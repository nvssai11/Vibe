...
const Login = () => {
  const { login, error, notice } = useAuth();
  ...
  const handleSubmit = async (e) => {
    ...
    if (user) {
      if (user.role === "resident" && user.status === "pending") {
        navigate("/dashboard", { state: { notice: "Awaiting admin approval" } });
      } else if (user.role === "apartment_admin" || user.role === "super_admin") {
        navigate("/admin");
      } else {
        navigate("/");
      }
  ...
  return (
    ...
    >
      ...
      <motion.div 
        ...
        className="absolute -left-20 -top-20 w-40 h-40 rounded-full bg-indigo-100 opacity-30"
      />
      <motion.div 
        animate={{ 
          x: [0, -80, 0],
        ...
        className="absolute -right-20 bottom-10 w-48 h-48 rounded-full bg-pink-100 opacity-30"
      />
      ...
      >
        ...
        >
          <motion.div 
            ...
            className="bg-gradient-to-r from-indigo-600 to-pink-500 p-4 rounded-full shadow-lg"
          >
            <FiUser className="h-12 w-12 text-white" />
          </motion.div>
        </motion.div>
        <h2 className="text-3xl font-bold mb-8 text-center bg-gradient-to-r from-indigo-600 to-pink-600 bg-clip-text text-transparent">Welcome Back to your Community</h2>
        ...
        <div className="mt-8 text-center text-sm text-gray-600">
          ...
          <p className="mt-4">
            <motion.button
               ...
               whileHover={{ scale: 1.05 }}
               whileTap={{ scale: 0.95 }}
               className="font-medium text-gray-600 hover:text-gray-500 hover:underline focus:outline-none"
             >
               Forgot password?
             </motion.button>
          </p>
